#!/usr/bin/env bash
# =============================================================================
# Dome Throughput Test Automation Script — iperf2 variant
#
# This is a port of the original iperf3-based script to iperf2 (binary: `iperf`,
# package `iperf` on Debian/Ubuntu). The motivation is that iperf3 maintains a
# TCP control channel between client and server even for UDP tests; over a
# degraded radio link emulated by PROPSIM, that control channel routinely
# drops at the end of the test with "unable to receive results". iperf2 has
# no such control channel — the UDP test is pure data-plane.
#
# Architectural change vs the iperf3 version:
#   * iperf3: server inside UPF, client on the UE host, downlink via -R
#   * iperf2: server on the UE host, client inside UPF, downlink is just
#             the natural direction (UPF → UE). No -R, no control channel.
#
#   This means per-interval and summary stats are captured on the UE side
#   (where the script runs), which is also the authoritative receiver.
#
# Session setup (once):
#   * Start gnb-cst (kept up across all iterations)
#   * The iperf2 server is started PER ITERATION on the UE host, because it
#     binds to ${IPERF_CLIENT} which only exists while ue-cst is up.
#
# Per iteration:
...
#   5. Start iperf2 server on the UE host bound to ${IPERF_CLIENT}
#   6. Run iperf2 client inside the upf container pushing UDP downlink
#   7. Stop the iperf2 server, rewind PROPSIM, stop ue-cst
#
# Session teardown:
#   * Stop and remove gnb-cst
#   * Kill any leftover iperf2 server on the UE host
#
# Usage:
#   ./throughput_dl_udp_iperf2.sh --name <test_name> [options]
# =============================================================================
...
PROPSIM_PORT=3334
IPERF_SERVER="192.168.100.1"     # UPF-side address (iperf2 client source)
IPERF_CLIENT="192.168.100.3"     # UE-side address (iperf2 server bind addr)
IPERF_PORT=5202
IPERF_TIME=100
IPERF_BANDWIDTH="40M"
UPF_CONTAINER="upf"
...
# =============================================================================
# PROPSIM ATE communication (unchanged from iperf3 version)
# =============================================================================
PROPSIM_CONNECTED=0
...
# =============================================================================
# iperf2 server on the UE host
#
# We start a fresh server PER ITERATION because the bind address
# (${IPERF_CLIENT}) only exists while ue-cst is up. PID is tracked so we can
# kill it cleanly even if the iperf2 process doesn't self-terminate.
# =============================================================================
IPERF_SERVER_PID=""

iperf_server_start_local() {
    local out_log="$1"

    # Kill any stale server on this host bound to the same port
    pkill -f "iperf -s.*-p ${IPERF_PORT}" 2>/dev/null || true
    sleep 0.3

    log "  Starting iperf2 server on UE host (bind ${IPERF_CLIENT}:${IPERF_PORT}, UDP)..."
    # -s server, -u UDP, -B bind, -p port, -i 1 per-second interval,
    # -f m report in Mbit/s, -e enhanced output (latency, jitter, loss)
    iperf -s -u -B "${IPERF_CLIENT}" -p "${IPERF_PORT}" -i 1 -f m -e \
        > "$out_log" 2>&1 &
    IPERF_SERVER_PID=$!
    sleep 0.5

    if kill -0 "$IPERF_SERVER_PID" 2>/dev/null; then
        log_ok "  iperf2 server running (pid ${IPERF_SERVER_PID}). Output → ${out_log}"
    else
        log_err "  iperf2 server failed to start. See ${out_log}:"
        sed 's/^/    /' "$out_log" >&2 || true
        return 1
    fi
}

iperf_server_stop_local() {
    if [[ -n "$IPERF_SERVER_PID" ]] && kill -0 "$IPERF_SERVER_PID" 2>/dev/null; then
        log "  Stopping iperf2 server (pid ${IPERF_SERVER_PID})..."
        # SIGINT lets iperf2 print its final summary, then exit cleanly
        kill -INT "$IPERF_SERVER_PID" 2>/dev/null || true
        # Give it a moment to flush the summary line
        for _ in 1 2 3 4 5; do
            kill -0 "$IPERF_SERVER_PID" 2>/dev/null || break
            sleep 0.3
        done
        kill -KILL "$IPERF_SERVER_PID" 2>/dev/null || true
        wait "$IPERF_SERVER_PID" 2>/dev/null || true
        log_ok "  iperf2 server stopped."
    fi
    # Belt and braces: anything else listening on this port
    pkill -f "iperf -s.*-p ${IPERF_PORT}" 2>/dev/null || true
    IPERF_SERVER_PID=""
}
...
command -v iperf  >/dev/null 2>&1 || { log_err "'iperf' (iperf2) not found. Install with: apt install iperf"; exit 1; }
...
# Verify the UPF container will have iperf2 available. We don't try to install
# it; we just fail fast if it's missing.
...
echo -e "  UPF container    : ${BOLD}${UPF_CONTAINER}${NC} (iperf2 CLIENT)"
echo -e "  iperf2 server    : ${BOLD}${IPERF_CLIENT}:${IPERF_PORT}${NC} (UE host)"
echo -e "  iperf2 client    : ${BOLD}${IPERF_SERVER}${NC} (inside UPF, sending downlink)"
...
for (( i=1; i<=ITERATIONS; i++ )); do
    ...
    log "Receiver (server) output → ${CSV_FILE}"
    log "Sender (client)   output → ${CLIENT_LOG}"
    ...
    # ----- 4. Start the iperf2 server on the UE host --------------------
    echo "# === Iteration ${i} — $(date '+%Y-%m-%dT%H:%M:%S') ===" >  "$CSV_FILE"
    echo "# Server: iperf -s -u -B ${IPERF_CLIENT} -p ${IPERF_PORT} -i 1 -f m -e" >> "$CSV_FILE"
    iperf_server_start_local "$CSV_FILE"

    # ----- 5. Run the iperf2 client from inside the UPF container -------
    # No control channel; the client just emits UDP datagrams toward the
    # server. With iperf2, exit status reflects only whether the client
    # process itself ran — there's no post-test handshake to fail. We
    # still capture stderr so we can spot socket-level problems.
    log "Step 5: Running iperf2 client inside '${UPF_CONTAINER}' (UDP, ${IPERF_BANDWIDTH}, ${IPERF_TIME}s)..."
    echo "# === Iteration ${i} — client side — $(date '+%Y-%m-%dT%H:%M:%S') ===" > "$CLIENT_LOG"
    echo "# Client: iperf -c ${IPERF_CLIENT} -B ${IPERF_SERVER} -u -b ${IPERF_BANDWIDTH} -p ${IPERF_PORT} -t ${IPERF_TIME} -i 1 -f m -e" >> "$CLIENT_LOG"

    iperf3_ok=0
    while [[ "$iperf3_ok" -eq 0 ]]; do
        if docker exec "${UPF_CONTAINER}" iperf \
                -c "${IPERF_CLIENT}" \
                -B "${IPERF_SERVER}" \
                -u \
                -b "${IPERF_BANDWIDTH}" \
